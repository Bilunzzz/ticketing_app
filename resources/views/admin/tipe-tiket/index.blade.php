            ...
            <table class="table">
                ...
                <tbody>
                    @forelse ($tipeTikets as $index => $tipeTiket)
                        <tr>
                            <th>{{ $index + 1 }}</th>
                            <td>{{ $tipeTiket->nama }}</td>
                            <td>
                                <button class="btn btn-sm btn-primary mr-2" onclick="openEditModal(this)" data-id="{{ $tipeTiket->id }}" data-nama="{{ $tipeTiket->nama }}">Edit</button>
                                <button class="btn btn-sm bg-red-500 text-white" onclick="openDeleteModal(this)" data-id="{{ $tipeTiket->id }}">Hapus</button>
                            </td>
                        </tr>
                        @empty
                        <tr>
    ...
    <dialog id="add_modal" class="modal">
        <form method="POST" action="{{ route('admin.tipe-tikets.store') }}" class="modal-box">
            ...
            <div class="form-control w-full mb-4">
                ...
                <input type="text" placeholder="Masukkan nama tipe tiket (contoh: Reguler, VIP, Early Bird)" class="input input-bordered w-full" name="nama" required />
            </div>
     ...
     <dialog id="edit_modal" class="modal">
        <form method="POST" class="modal-box">
            ...
            <div class="form-control w-full mb-4">
                ...
                <input type="text" placeholder="Masukkan nama tipe tiket" class="input input-bordered w-full" id="edit_tipe_tiket_name" name="nama" />
            </div>
            <div class="modal-action">
                <button class="btn btn-primary" type="submit">Simpan</button>
